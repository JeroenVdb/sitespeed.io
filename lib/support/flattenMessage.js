...
module.exports = {
  ...
  flattenMessageData(message) {
    ...
    function recursiveFlatten(target, keyPrefix, value, type) {
      ...
      if(cli.parseCommandLine().options.userTimingWhitelist) {
        let cliWhitelistRegex =  cli.parseCommandLine().options.userTimingWhitelist;
        let userTimingsRegex = new RegExp("statistics\\.timings\\.userTimings\\.marks\\." + cliWhitelistRegex);
        let userTimesRegex = new RegExp('userTimings\\.marks\\.' + cliWhitelistRegex);
        if(keyPrefix.indexOf('.marks.') > -1 && ((userTimingsRegex.exec(keyPrefix)) === null || (userTimesRegex.exec(keyPrefix)) === null)) {
            return;
        }
      } else {
